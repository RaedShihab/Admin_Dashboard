...
class AccountDetails extends React.Component {
    constructor(props) {
      ...
      componentDidMount() {
        ...
        Axios.get('/locations/cities').then(res => {
       ...
       update && 
       Axios.get(`/locations/districts/${data.id}`).then(res => {
    ...
    render() {
      ...
      const { formLoading, enName, arName, lon, lat, order, countries, select, filterData, cities} = this.state;

      const dataList = cities.map(item => {return {name: item.name.en, id: item.id}});
      const defaultProps = {
        options: dataList,
                ...
                initialValues={{
                  ...
                  onSubmit={data => {
                    ...
                    let values = new FormData();
                        ...
                        values.append('city_id', data.select);
                    ...
                    this.props.request(values)
                           ...
                           .catch(err => {
                             ...
                             if(err.response.status === 422) {
                               if(err.response.data.city_id !== undefined) {
                                this.setState({
                                          ...
                                          <Grid
                                            ...
                                            >
                                              <Autocomplete
                                                  ...
                                                  disableOpenOnFocus
                                                  onChange={(e, value) => {props.setFieldValue("select", value.id); }}
                                                  renderInput={params => (
                                                    <TextField
